...
def home_page(request):
    ...
    context = {'rooms': [member.room for member in Member.objects.filter(user=request.user)]}
    return render(request, 'home/home.html', context=context)


def room(request, invite_link):
    if not request.user.is_authenticated:
        return HttpResponseRedirect(reverse('home:index'))
    ...
    return render(request, 'home/chats.html', context=context)
...
def send(request):
    ...
    message = Message(sender=sender, text=text, room=r, date=timezone.now())
    message.save()
...
def sign_up(request):
    if 'code' not in request.POST:
        ...
        if not re.findall('^[a-zA-Z][a-zA-Z0-9_]{4,30}$', username):
            ...
            return HttpResponseRedirect(reverse('home:index'))
        if User.objects.filter(username=username) or User.objects.filter(telegram_id=telegram_id):
